...
public class PlayerEJB implements PlayerEJBLocal{
	...
	public Object[] getSumOfPlayerStatistics(Player player, boolean onlyPlayoffs) {
		String qlString = "SELECT SUM(ps.gamesPlayed)" + ",SUM(ps.goals)" + ",SUM(ps.assists)" + ",SUM(ps.points)"
				+ ",SUM(ps.plusminus)" + ",SUM(ps.penaltyInMinutes)" + ",SUM(ps.powerPlayGoals)"
				+ ",SUM(ps.powerPlayPoints)" + ",SUM(ps.shortHandedGoals)" + ",SUM(ps.shortHandedPoints)"
				+ ",SUM(ps.gameWinningGoals)" + ",SUM(ps.overtimeGoals)" + ",SUM(ps.shots)" +
				// TODO round up to 2 decimal points
				",AVG(ps.shotPercentage)" + ",AVG(ps.faceoffWinPercentage)" + "FROM PlayerStatistics ps "
				+ "WHERE ps.player = :PLAYER AND " + "ps.playoffStatistics = :ONLYPLAYOFFS";
